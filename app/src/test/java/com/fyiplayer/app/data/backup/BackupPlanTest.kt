...
class BackupPlanTest {
    ...
    private fun sampleDoc() = BackupDocument(
        ...
        history = listOf(BackupVideo("youtube", "https://example.com/v=d", "D")),
    )
    ...
    private fun snapshotAfterApplying(doc: BackupDocument): Triple<Map<String, Set<String>>, Set<String>, Set<String>> =
        Triple(
            ...
            doc.history.map { it.pageUrl }.toSet(),
        )

    @Test
    fun `first import against an empty library plans everything`() {
        val plan = planImport(sampleDoc(), emptyMap(), emptySet(), emptySet())
        assertEquals(BackupPlan(newPlaylists = 1, newPlaylistItems = 2, newLiked = 1, newHistory = 1), plan)
    ...
    fun `re-importing the same document after it was applied plans nothing`() {
        val doc = sampleDoc()
        val (playlistItems, liked, history) = snapshotAfterApplying(doc)

        val second = planImport(doc, playlistItems, liked, history)
    ...
    fun `applying twice yields the same plan as applying once`() {
        ...
        val second = planImport(doc, playlistItems, liked, history)
        val third = planImport(doc, playlistItems, liked, history)
    ...
    fun `partial overlap only counts what is actually new`() {
        ...
        val existingHistory = emptySet<String>() // d is new

        val plan = planImport(doc, existingPlaylistItems, existingLiked, existingHistory)

        assertEquals(BackupPlan(newPlaylists = 0, newPlaylistItems = 1, newLiked = 0, newHistory = 1), plan)
    }
}
